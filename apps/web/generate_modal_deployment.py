#!/usr/bin/env python3
"""
Generate Modal deployment file with embedded Vite assets
"""
...
frontend_app = FastAPI(title="twoby - Real Vite Frontend")
...
def main():
    ...
    print("Reading Vite assets from dist/...")
    assets = generate_asset_dict()
    
    print(f"Found {len(assets)} assets:")
    for path in sorted(assets.keys()):
        size = len(assets[path]) if isinstance(assets[path], str) else len(assets[path]['content'])
