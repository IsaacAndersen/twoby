...
export default function ShareOverlay({
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver((entries) => {
      ...
      chart.style.transform = `scale(${width / 1080})`
    })
    observer.observe(container)
    return () => observer.disconnect()
  }, [])

  // Clear render cache when items change
  useEffect(() => {
    clearRenderCache()
  }, [items])
  ...
  const getShareUrl = useCallback(async () => {
    ...
    params.set('s', shareKey)
    try {
      return await createShortUrl(`/c/${chartId}`, params, title)
    } catch {
      return `${window.location.origin}/c/${chartId}?s=${shareKey}`
    }
  ...
  async function handleDownload() {
    ...
    const safeName = title.replace(/[^a-z0-9]/gi, '-').toLowerCase().slice(0, 40)
    downloadBlob(blob, `${safeName}-twoby.png`)
  }
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-lg rounded-2xl bg-white shadow-2xl overflow-hidden">
        ...
        <button
          onClick={onClose}
          className="absolute top-3 right-3 z-10 flex h-8 w-8 items-center justify-center rounded-full bg-stone-100 text-stone-500 hover:bg-stone-200 hover:text-stone-700 transition-colors"
          aria-label="Close"
        >
          <X size={16} />
        </button>
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="flex gap-2">
            <Button
              variant="ghost"
              className="flex-1 flex items-center justify-center gap-2 text-stone-600"
